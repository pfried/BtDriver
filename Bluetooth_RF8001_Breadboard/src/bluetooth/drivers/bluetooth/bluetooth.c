...
void bluetooth_init(bluetooth_config_t *bluetooth_config, bluetooth_car_t *bluetooth_car) {
	...
	*/
	...
	aci_state.aci_pins.interface_is_interrupt = BLUETOOTH_INTERFACE_IS_INTERRUPT;
...
void bluetooth_process(void) {
				...
				*/
				if (false == timing_change_done)
				{
					...
					*/
					...
					actors_on_pipe_status(&aci_state, &car->horn, &car->lights, &car->generic_actor_1, &car->generic_actor_2);
					sensors_on_pipe_status(&aci_state, &car->brightness, &car->temperature, &car->battery);
	...
	if(setup_required)
	{
		if (SETUP_SUCCESS == do_aci_setup(&aci_state))
		{
			...
			sensors_init(&oldcar.brightness, &oldcar.temperature, &oldcar.battery);
			speed_and_angle_init(&oldcar.speed, &oldcar.direction, &oldcar.sensorServo);
			distance_init(&oldcar.distance_ir_front, &oldcar.distance_ir_rear, &oldcar.distance_us_front, &oldcar.distance_us_rear);
	...
	if(timing_change_done == true) {
		...
		if(bluetooth_process_flag == true) {
			...
			switch(bt_counter) {
				...
				case PIPE_SENSORS_SENSORS_TX:
				{
					sensors_update(&aci_state, &car->brightness, &car->temperature, &car->battery);
					break;
				}
				case PIPE_DISTANCE_DISTANCE_TX:
				{
					distance_update(&aci_state, &car->distance_ir_front, &car->distance_ir_rear, &car->distance_us_front, &car->distance_us_rear);
					break;
				}
				case PIPE_ACTORS_ACTORS_TX:
				{
					actors_update(&aci_state, &car->horn, &car->lights, &car->generic_actor_1, &car->generic_actor_2);
					break;
				}
				case PIPE_DRIVE_SPEEDANDANGLE_TX:
				{
					speed_and_angle_update(&aci_state, &car->speed, &car->direction, &car->sensorServo);
					break;
				}
			...
			// Increment the counter only if there is a credit left
